...
export function buildToolPageLensContent(
  input: BuildToolPageLensContentInput
): ToolPageLensContent {
  const verdictLabel =
  ...
  const lensWeakFitLine = (() => {
    if (input.activeReviewLens === 'personal') {
      return (
        input.decisionSnapshotWatchOuts[0] || 'Buyers who need contract-heavy enterprise controls.'
      );
    }
    if (input.activeReviewLens === 'startup') {
      return (
