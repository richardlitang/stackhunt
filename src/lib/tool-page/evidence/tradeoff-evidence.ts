...
import { isEligibleEvidenceUrl } from '@/lib/tool-page/evidence/evidence-policy';
...
export function buildToolPageTradeoffEvidence(
  ...
  const avoidIfBullet = buildToolPageEvidenceBulletV2({
    text:
      input.decisionSnapshotWatchOuts[0] || 'Your workflow cannot tolerate unresolved constraints.',
    kind: 'tradeoff',
