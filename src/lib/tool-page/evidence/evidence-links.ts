import { isBlockedEvidenceDomain } from '@/lib/tool-page/evidence/evidence-policy';
...
export function buildToolPageEvidenceLinks(
  ...
  const evidenceLinksAll = sources
    .map((source) => {
      ...
      const blocked = isBlockedEvidenceDomain(hostname);
      ...
      const isLikelyOffTopicOfficial =
        sourceType === 'official' && path.includes('/blog/') && !toolSignal;
