...
export function buildToolPageSectionState(
  input: BuildToolPageSectionStateInput
): ToolPageSectionState {
  const evidenceContract = createToolPageEvidenceContract({
    ...
    lastCheckedByField: {
      evidence:
        input.firstReviewUpdatedAt ||
        input.firstReviewCreatedAt ||
        input.toolLastVerifiedAt ||
        null,
