...
export const SMPPlanSchema = z
  .object({
    ...
    scaling_unit: z
      ...
      .optional()
      .describe(
        'The noun being counted. Use standard units (user, contact, gb) where possible, but preserve unique terms (zap, token, credit) if the tool uses them.'
      ),
...
export const SMPPricingDataSchema = z.object({
  ...
  is_standalone: z
    .boolean()
    .describe('False if primarily sold as part of a suite (e.g., Google Meet, Teams)')
    .default(true),
  bundled_in: z
    .string()
    .nullable()
  ...
  discounts_available: z
    .array(z.enum(['startup', 'nonprofit', 'education', 'government', 'annual_prepay']))
    .default([]),
...
export const SMPTaxonomySchema = z.object({
  ...
  sub_category: z
    ...
    .optional()
    .describe(
      'Technical sub-category for comparison granularity. Examples: "CPaaS" vs "Team Chat", "Infrastructure API" vs "SaaS Application", "Marketing Automation" vs "Email Client"'
    ),
...
export const ConstraintSchema = z.object({
  ...
  overage: z
    .object({
      // Overage costs for auto_charge limits
      cost: z.number(),
...
export const BudgetAnalystSchema = z.object({
  cost_drivers: z
    .array(z.string())
    .describe(
      "Factual factors that increase TCO. e.g., 'SSO requires Enterprise', 'Guests are billable', 'Storage overage fees'"
    )
    .default([]),
  one_time_fees: z
    .array(z.string())
...
export const UserAdvocateSchema = z.object({
  vibe: z
    .string()
    .describe(
      "Ecosystem maturity & build style. e.g., 'No-Code Modular', 'Enterprise Rigid', 'Developer-First', 'Builder-Focused'"
    )
    .nullable()
    .optional(),
  origin_story: z
  ...
  avoid_if: z
    .array(z.string())
    .describe(
      "Deal-breakers. e.g., 'Need HIPAA compliance', 'Offline-heavy workflow', 'You hate keyboard shortcuts'"
    )
    .default([]),
  power_tip: z
    .string()
    .nullable()
    .describe(
      "One specific 'insider' shortcut or feature regular users might miss. e.g., 'Use Cmd+K to navigate'"
    )
    .optional(),
  delighters: z
    .array(z.string())
    .describe(
      "Specific features users rave about. e.g., 'The command palette', 'Dark mode', 'Real-time collaboration'"
    )
    .default([]),
  frustrations: z
    .array(z.string())
    .describe(
      "Specific UX complaints (NOT price complaints, those go in Budget Analyst). e.g., 'Search is slow', 'Mobile app is buggy'"
    )
    .default([]),
});
...
export const KnowledgeCardSchema = z.object({
  ...
  company: z
    .object({
      ...
      employee_count: z
        ...
        .optional(),
      funding_stage: z
        .enum(['bootstrapped', 'seed', 'series-a', 'series-b', 'series-c+', 'public', 'acquired'])
        .nullable()
        .optional(),
    })
    .default({}),
  ...
  security: z
    .object({
      ...
      data_encryption: z
        .enum(['at-rest', 'in-transit', 'both', 'none', 'unknown'])
        .nullable()
        .optional(),
...
export const GeminiKnowledgeCardSchema = {
  ...
  properties: {
    ...
    platforms: {
      ...
      items: {
        ...
        properties: {
          platform: {
            type: 'string',
            enum: ['web', 'mac', 'windows', 'linux', 'ios', 'android', 'cli', 'api', 'self-hosted'],
          },
    ...
    audience: {
      ...
      properties: {
        ...
        team_size: {
          type: 'string',
          nullable: true,
          enum: ['solo', 'small', 'medium', 'enterprise', 'any'],
        },
        skill_level: {
          type: 'string',
          nullable: true,
    ...
    smp_pricing: {
      type: 'object',
      description:
        'Structured pricing data for cost calculations. Extract pricing LOGIC, not just strings.',
      properties: {
        model: {
          type: 'string',
          description:
            'The pricing model. Valid values: free, flat (Basecamp $99/mo), per_seat (Slack $8/user), per_unit (Twilio per message), tiered (HubSpot), hybrid (Notion free + per user), contact_sales (enterprise), ad_spend (Google Ads, Meta Ads - CPC/CPM bidding), usage_based (OpenAI API, AWS - pay per token/GB/request)',
        },
        // Suite bundling (V3.2)
        is_standalone: {
          type: 'boolean',
          default: true,
          description:
            'False if primarily sold as part of a suite (e.g., Google Meet, Microsoft Teams cannot be purchased alone)',
        },
        bundled_in: {
          type: 'string',
          nullable: true,
        ...
        billing_cycles: {
          ...
          description: 'Available billing options',
        },
        annual_discount_pct: {
          type: 'number',
          nullable: true,
          description: 'Percentage discount for annual billing (e.g., 20 means 20% off)',
        },
        plans: {
          type: 'array',
          ...
          items: {
            ...
            properties: {
              id: {
                type: 'string',
                description: 'Plan identifier: tool-slug-plan-name-slug (e.g., "notion-plus")',
              },
              ...
              price_monthly: {
                ...
                nullable: true,
                description:
                  'Monthly price in base currency units (e.g., 10 for $10/mo). null if no monthly option.',
              },
              price_annual: {
                type: 'number',
                nullable: true,
                description: 'TOTAL annual price (e.g., 96 for $96/year). NOT monthly equivalent.',
              },
              scaling_unit: {
                type: 'string',
                nullable: true,
                description:
                  'What the price scales with: user, seat, member, GB, message, request, project, workspace, hour',
              },
              price_per_unit: {
                type: 'number',
                nullable: true,
                description: 'Price per unit for per_seat/per_unit models',
              },
              included_units: {
                type: 'number',
                nullable: true,
                description: 'Units included in base price (e.g., "includes 5 users")',
              },
              max_users: {
                type: 'number',
                nullable: true,
                description: 'User limit for this plan. null = unlimited.',
              },
              ...
              is_enterprise: {
                type: 'boolean',
                default: false,
                description: 'true if this is "Contact Sales" pricing',
              },
              ...
              variable_unit: {
                ...
                nullable: true,
                description:
                  'Unit for variable pricing: "click", "1k impressions", "1M tokens", "GB", "API call". Only for ad_spend/usage_based models.',
              },
              variable_price: {
                type: 'number',
                nullable: true,
                description:
                  'Price per variable_unit (e.g., 0.50 for "$0.50 per click"). Only for ad_spend/usage_based models.',
              },
              variable_logic_desc: {
                type: 'string',
                nullable: true,
                description:
                  'Human-readable explanation of variable pricing (e.g., "Bidding based, typically $0.40-$2.00 CPC depending on targeting"). Only for ad_spend/usage_based models.',
              },
            },
        ...
        min_seats: {
          type: 'number',
          nullable: true,
          description: 'Minimum seat purchase (e.g., "Min 5 seats")',
        },
        implementation_fee: {
          type: 'number',
          nullable: true,
          description: 'One-time setup/implementation fee',
        },
        pricing_page_url: {
          type: 'string',
          nullable: true,
          description: 'URL to official pricing page for verification',
        },
        confidence: {
          type: 'string',
          enum: ['high', 'medium', 'low'],
          description:
            'How confident are you in this pricing data? high = from official pricing page, medium = from review sites, low = inferred or outdated',
        },
        discounts_available: {
          type: 'array',
          items: {
            type: 'string',
            enum: ['startup', 'nonprofit', 'education', 'government', 'annual_prepay'],
          },
        },
      },
    ...
    smp_taxonomy: {
      ...
      properties: {
        primary_function: {
          type: 'string',
          description:
            'Primary function: "Project Management", "CRM", "Communication", "Documentation", etc.',
        },
        sub_category: {
          type: 'string',
          nullable: true,
        ...
        secondary_functions: {
          type: 'array',
          items: { type: 'string' },
          description: 'Secondary functions the tool also serves',
        },
        likely_departments: {
          type: 'array',
          items: { type: 'string' },
          description:
            'Departments that typically own/pay for this tool: "Engineering", "Product", "Marketing", "Sales", "Operations", "Finance", "HR", "Legal", "IT Security", "Customer Success"',
        },
      },
    ...
    smp_portability: {
      ...
      properties: {
        ...
        export_formats: {
          type: 'array',
          items: { type: 'string' },
          description: 'Available export formats: "CSV", "JSON", "XML", "PDF", "ZIP"',
        },
        has_api_export: {
          type: 'boolean',
          description: 'Can users programmatically extract all their data via API?',
        },
        migration_difficulty: {
          type: 'string',
          nullable: true,
          enum: ['trivial', 'easy', 'moderate', 'hard', 'locked'],
          description: 'How hard is it to migrate away from this tool?',
        },
        import_from: {
          type: 'array',
          items: { type: 'string' },
          description: 'Tool names that have import wizards INTO this tool',
        },
        export_to: {
          type: 'array',
          items: { type: 'string' },
          description: 'Tool names that have import wizards FROM this tool',
        },
        min_commitment_months: {
          type: 'number',
          nullable: true,
          description: 'Minimum contract commitment in months. null = month-to-month.',
        },
        cancellation_notice_days: {
          type: 'number',
          nullable: true,
          description: 'Required notice period for cancellation in days',
        },
      },
    },
    // V4: Constraints (The "Cynical CTO" Layer)
    ...
    review_context: {
      type: 'object',
      description:
        'Tribal knowledge, vibe, and opinionated guidance extracted from Reddit, forums, and honest user reviews.',
      properties: {
        human_verdict: {
          type: 'string',
          nullable: true,
        ...
        budget_analyst: {
          ...
          properties: {
            cost_drivers: {
              ...
              items: { type: 'string' },
              description:
                'Factual factors that increase TCO. Examples: "SSO requires Enterprise tier", "Guests count as billable users", "Storage overage fees after 100GB", "Implementation fee for Enterprise"',
            },
            one_time_fees: {
              type: 'array',
              items: { type: 'string' },
              description:
                'One-time costs. Examples: "$500 implementation fee", "Mandatory onboarding training at $200/hr"',
            },
            commitment_terms: {
              type: 'string',
              nullable: true,
              description:
                'Contract constraints. Examples: "Annual only, no monthly option", "30-day cancellation notice required", "Auto-renews unless cancelled 60 days prior"',
            },
            roi_threshold: {
              type: 'string',
              nullable: true,
        ...
        user_advocate: {
          type: 'object',
          description:
            'The Senior Engineer perspective: Vibe, tribal knowledge, and honest experience',
          properties: {
            vibe: {
              type: 'string',
              nullable: true,
              description:
                'Ecosystem maturity & build style (2-3 words). Examples: "No-Code Modular" (Airtable), "Enterprise Rigid" (SAP), "Developer-First" (Stripe), "Builder-Focused" (Webflow), "API-Centric" (Twilio), "Workflow Engine" (n8n). AVOID fluff like "Hacker Chic" or "Playful".',
            },
            origin_story: {
              type: 'string',
              nullable: true,
              description:
                'One sentence on context. Examples: "Started as game chat for WoW guilds, now used by startups", "Built by designer frustrated with Jira", "Originally an internal tool at Spotify"',
            },
            ideal_for: {
              type: 'array',
              items: { type: 'string' },
              description:
                'Specific personas who thrive. Examples: "Solo founders", "Async-first remote teams", "Design teams at scale", "Developers who love keyboard shortcuts"',
            },
            avoid_if: {
              type: 'array',
              items: { type: 'string' },
              description:
                'Deal-breaker scenarios. Examples: "You need offline access", "You work in a regulated industry (HIPAA)", "Your team hates keyboard shortcuts", "You prefer visual/GUI workflows"',
            },
            power_tip: {
              type: 'string',
              nullable: true,
              description:
                'One specific insider shortcut or hidden feature. Examples: "Use /collapse to hide all gifs in Slack", "Cmd+K opens command palette", "Enable vim mode in settings"',
            },
            delighters: {
              type: 'array',
              items: { type: 'string' },
              description:
                'Specific features users rave about. Examples: "The command palette is chef\'s kiss", "Dark mode actually looks good", "Real-time collaboration just works"',
            },
            frustrations: {
              type: 'array',
              items: { type: 'string' },
              description:
                'Specific UX complaints (NOT pricing). Examples: "Search is painfully slow after 10k messages", "Mobile app crashes frequently", "No offline mode"',
            },
          },
        },
      },
...
export function getComparableFacts(card: KnowledgeCard): Record<string, unknown> {
  return {
    ...
    'Free Trial': card.pricing.has_free_trial
    ...
    SSO:
      card.security.sso_available === true
        ...
        : card.security.sso_available === false
          ...
          : 'Unknown',
    '2FA':
      card.security.two_factor === true
        ? 'Yes'
        : card.security.two_factor === false
          ? 'No'
          : 'Unknown',
    'SOC 2':
