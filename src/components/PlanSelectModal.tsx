...
export default function PlanSelectModal({
  ...
  const [selectedPlanId, setSelectedPlanId] = useState<string>(() => {
    ...
    const defaultPlan = plans.find(p => !p.is_enterprise && (p.price_monthly != null || p.price_annual != null));
  ...
  const handleConfirm = () => {
    ...
    const pricePerUnit = billingCycle === 'annual' && selectedPlan.price_annual != null
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-md bg-white">
        <DialogHeader>
          ...
          <DialogDescription>
            Select your plan to calculate accurate costs
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-6 py-4">
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-2">
              {sortedPlans.map((plan) => {
                ...
                const displayPrice = plan.price_monthly != null
                ...
                return (
                  ...
                  >
                    ...
                    {plan.includes_sso && (
                      <span className="text-xs bg-slate-100 text-slate-600 px-2 py-0.5 rounded">SSO</span>
                    )}
                  </label>
                );
              })}
          ...
          {isPerSeat && selectedPlan && !selectedPlan.is_enterprise && selectedPlan.price_monthly !== 0 && (
            <div className="space-y-2">
              <label htmlFor="seats" className="text-sm font-medium text-slate-900 flex items-center gap-2">
          ...
          {/* Billing Cycle */}
          {selectedPlan && !selectedPlan.is_enterprise && (selectedPlan.price_monthly || selectedPlan.price_annual) && (
            <div className="space-y-2">
              ...
              <div className="flex gap-4">
                <label className={cn(
                  ...
                  <div className={cn(
                    ...
                    billingCycle === 'monthly' ? "border-emerald-500 bg-emerald-500" : "border-slate-300"
                  )}>
                    {billingCycle === 'monthly' && <div className="h-1.5 w-1.5 rounded-full bg-white" />}
                  </div>
                ...
                <label className={cn(
                  ...
                  <div className={cn(
                    "h-4 w-4 rounded-full border-2 flex items-center justify-center",
                    billingCycle === 'annual' ? "border-emerald-500 bg-emerald-500" : "border-slate-300"
                  )}>
                    {billingCycle === 'annual' && <div className="h-1.5 w-1.5 rounded-full bg-white" />}
                  </div>
                  <span className="text-sm">Annual</span>
                  {selectedPlan.price_annual && selectedPlan.price_monthly && (
                    <span className="text-xs text-emerald-600">
                      Save {Math.round((1 - (selectedPlan.price_annual / (selectedPlan.price_monthly * 12))) * 100)}%
                    </span>
                  )}
                </label>
              </div>
          ...
          <div className="rounded-lg bg-slate-50 p-4">
            ...
            {isPerSeat && seats > 1 && calculatedCost > 0 && (
              <p className="text-xs text-slate-500 mt-1">
                {seats} {selectedPlan?.scaling_unit || 'seat'}s × ${(calculatedCost / seats).toFixed(2)}/{selectedPlan?.scaling_unit || 'seat'}
              </p>
            )}
          </div>
        </div>
