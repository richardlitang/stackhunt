...
describe('tool page pricing notice', () => {
  it('includes checked label when available', () => {
    expect(buildToolPagePricingNotice({ pricingCheckedLabel: 'today' })).toContain('Shown as of today');
  });

  it('returns pending-check notice when label missing', () => {
    expect(buildToolPagePricingNotice({ pricingCheckedLabel: null })).toContain('Pricing check is pending');
  });
});
